...
The `zsim-bsc/` tree is a heavily extended ZSim fork, and it is the only version that can correctly reproduce the results in this paper. Not just any ZSim version will work.
...
This repository includes the simulator source trees, but not all runtime bundles by default. `./setup.sh` and `./scripts/setup-env.sh` will try to discover dependencies automatically. If Pin/HDF5 are missing, configure the dependency URLs in `scripts/setup-env.sh` or install them manually.
