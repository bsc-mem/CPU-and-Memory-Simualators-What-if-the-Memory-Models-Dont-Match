...
MemObject* BuildMemoryController(Config& config, uint32_t lineSize, uint32_t frequency, uint32_t domain, g_string& name) {
    ...
    std::vector<uint32_t> trackedCores;
    if (config.exists("sys.mem.trackCores")) {
        string trackedCoreStr = config.get<const char*>("sys.mem.trackCores");
        if (!trackedCoreStr.empty()) {
            trackedCores = ParseList<uint32_t>(trackedCoreStr, " ,");
        }
    }
    ...
    if (type == "Simple") {
        ...
        string application = "stream";//config.get<const char*>("sim.stats");
        ...
        bool useClockDivider = config.get<bool>("sys.mem.ramulatorOrgUseClockDivider", false);
        string application = "stream";//config.get<const char*>("sim.stats");
        mem = new RamulatorOrg(ramulatorConfig, zinfo->numCores, lineSize, latency, domain, name, pimMode, application, frequency, record_memory_trace, networkOverhead, useClockDivider, trackedCores);
        zinfo->simEndHandlers->push_back([mem]() {((RamulatorOrg*)mem)->finish();});
    } 
    #endif
    #ifdef _WITH_RAMULATOR2_
    else if (type == "Ramulator2") {
       auto ramulator2Config = config.get<const char*>("sys.mem.ramulator2Config");
       auto estimatorType = config.get<const char*>("sys.mem.boundPhaseLatencyEstimator", "fix");
...
Network* BuildNetwork(Config& config) {
    ...
    string noc_config_path = config.get<const char*>("sys.network.networkConfig", "");
    string noc_type = config.get<const char*>("sys.network.type", "");
    if (noc_type == "") return nullptr;

    if (noc_type == "FixLatency") {
        network = new FixedDelayNetwork(noc_config_path.c_str());
    } else if (noc_type == "MeshMD1")  {
        network = new MeshMD1Network(noc_config_path.c_str());
    } else {
        panic("unknown network type: %s", noc_type.c_str());
    }
    return network;
}
...
static void InitSystem(Config& config) {
    ...
    Network* network = BuildNetwork(config);
...
void SimInit(const char* configFile, const char* outputDir, uint32_t shmid) {
    ...
    pid_t harnessPid = getppid(); // fallback
    if (const char* s = getenv("ZSIM_HARNESS_PID")) {
        harnessPid = (pid_t)atoi(s);
    }
    zinfo->harnessPid = harnessPid;

    getLibzsimAddrs(&zinfo->libzsimAddrs);
    // Also collect other shared libraries we care about for debugging
    collectDebugLibs(&zinfo->debugLibs);
    if (zinfo->attachDebugger) {
        // Expose the debug libs info through the secondary pointer for the harness
        gm_set_secondary_ptr(&zinfo->debugLibs);
    ...
    zinfo->simEndHandlers = new g_vector<std::function<void()>>();
